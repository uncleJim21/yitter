...
async function callMakeClip(data) {
  ...
  // Fetch the video URL once and use it for all clips
  ...
  for (const [index, range] of clipRanges.entries()) {
      ...
      const clipCommand = `ffmpeg -i "${videoUrl}" -ss ${range.start} -to ${range.end} -c copy "${outputPath}"`;
  ...
  const mergeCommand = `ffmpeg -f concat -safe 0 -i "${listFilePath}" -c copy "${mergedFilePath}"`;

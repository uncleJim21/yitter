...
async function callMakeClip(data) {
  return new Promise((resolve, reject) => {
    ...
    // Step 2: Download the video using youtube-dl
    const command = `youtube-dl -f 'bestvideo[height<=720]+bestaudio/best[height<=720]' --merge-output-format mp4 -o ${videoPath} https://www.youtube.com/watch?v=${videoId}`;
    ...
    exec(command, (error, stdout, stderr) => {
      ...
      // Check if the video file exists after download
      if (!fs.existsSync(videoPath)) {
        console.error(`Downloaded video file does not exist: ${videoPath}`);
        reject({ error: 'Downloaded video file does not exist' });
        return;
      }
      ...
      const promises = clipRanges.map((range, index) => {
        return new Promise((innerResolve, innerReject) => {
          ...
          ffmpeg(videoPath)
            ...
            .on('error', (err) => {
              console.error(`Error processing clip ${index + 1}:`, err);
              innerReject(err);
            })
            .run();
        });
